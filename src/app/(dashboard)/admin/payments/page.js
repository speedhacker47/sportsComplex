...
import { CreditCard, History, User, Building2, CheckCircle, AlertCircle, X, Search, Calendar, Filter } from 'lucide-react';
...
const PaymentsPage = () => {
  ...
  const [filterPeriod, setFilterPeriod] = useState('all'); // daily, monthly, yearly, all
      ...
      const paymentsSnap = await getDocs(query(paymentsRef, orderBy('paymentDate', 'desc')));
       ...
       const paymentsData = await Promise.all(
        paymentsSnap.docs.map(async (paymentDoc) => {
          ...
          if (payment.userId && typeof payment.userId.get === 'function') {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
        ...
        {/* Second Row - Facility and Period */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <select
        ...
        {/* Third Row - Date Selection (conditional) */}
        {filterPeriod !== 'all' && (
